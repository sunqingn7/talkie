...
class TTSTool(BaseTool):
    ...
    def __init__(self, config: dict):
        ...
        # Engine management
        self.current_engine = self.tts_config.get(
            "engine", "edge_tts"
        )  # Default to edge-tts
        self.coqui_tts = None
    ...
    def warmup(self):
        """Warm up TTS engine by generating a short audio clip in background."""
        import threading
    ...
    def _do_warmup(self):
        ...
        try:
            ...
            if result.get("success"):
                ...
                print(f"[TTS] Warmup failed: {result.get('error')}")
        except Exception as e:
    ...
    def _preprocess_chinese_text(self, text: str) -> str:
        ...
        text = re.sub(
            r"[a-zA-Z]+", "字母", text
        )  # Replace English words with "letters"
    ...
    def _init_pyttsx3(self):
        ...
        try:
            ...
            self.pyttsx3_engine.setProperty(
        ...
        for player_cmd, player_name in players:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    print(
                        f" [AUDIO DEBUG] Started {player_name} with PID {process.pid}"
                    )
    ...
    def stop_audio(self, reason: str = "general") -> bool:
        ...
        print(
            f" [AUDIO DEBUG] TTSTool stop_audio called. reason={reason}, audio_type={self.current_audio_type}, current_process: {self.current_audio_process}"
        )
        ...
        if reason == "chat" and self.current_audio_type == "file":
            print(
                f" [AUDIO DEBUG] TTSTool: Skipping stop - current audio is file reading"
            )
            return False
        ...
        if self.edge_tts_tool and hasattr(self.edge_tts_tool, "stop_audio"):
            try:
                print(
                    f" [AUDIO DEBUG] Stopping EdgeTTS tool audio with reason={reason}"
                )
    ...
    async def execute(
        ...
        Args:
            ...
            on_chunk: Optional callback function called with (audio_file, chunk_index, total_chunks) after each chunk is generated
        """
        ...
        print(
            f"   [TTS] Executing TTS: {len(text)} chars, engine: {self.current_engine}, language: {language}"
        )
        ...
        try:
            ...
            if self.current_engine == "qwen_tts":
                ...
                if self.qwen_tts_tool:
                    return await self._speak_qwen(
                        text, language, speed, audio_type, on_chunk
                    )
            ...
            if self.current_engine == "coqui" or (
                self.current_engine == "edge_tts" and self.edge_tts_tool is None
            ):
                ...
                if self.coqui_tts is not None or self._init_coqui_tts(language):
                    ...
                    if language != self.current_language:
                        if not self._switch_model(language):
                            print(
                                f"   ⚠️  Could not load {language} model, using current model"
                            )
    ...
    async def _speak_coqui(
        self, text: str, language: str, speaker_id: str = None, speed: float = 1.0
    ) -> Dict[str, Any]:
                ...
                if is_xtts:
                    ...
                    await loop.run_in_executor(
                        ...
                        lambda: self.coqui_tts.tts_to_file(
                            text=text,
                            speaker=speaker_id or self.coqui_tts.speakers[0]
                            if hasattr(self.coqui_tts, "speakers")
                            and self.coqui_tts.speakers
                            else None,
                            language=language
                            if language in self.XTTS_LANGUAGES
                            else "en",
                    ...
                    await loop.run_in_executor(
                        None,
                        lambda: self.coqui_tts.tts_to_file(
                            text=text, file_path=audio_file
                        ),
                    )
            finally:
                # Restore stderr
    ...
    async def _speak_qwen(
        ...
        # Only chunk for very long text (skip chunking for now to avoid blocking)
        # TODO: Implement proper async chunking
        ...
        if len(chunks) == 1:
            ...
            result = await self.qwen_tts_tool.execute(
                text=text, language=qwen_language, audio_type=audio_type
            )
        else:
            ...
            for i, chunk in enumerate(chunks):
                print(
                    f"[TTS] Processing chunk {i + 1}/{len(chunks)}: {len(chunk.split())} words"
                )
                result = await self.qwen_tts_tool.execute(
                    text=chunk, language=qwen_language, audio_type=audio_type
                )
                all_results.append(result)
                if result.get("success") and result.get("audio_file"):
                    audio_file = result["audio_file"]
                    all_audio_files.append(audio_file)
                    # Call on_chunk callback if provided (for streaming broadcast)
                    if on_chunk:
                        try:
                            # Check if callback is coroutine (async) and await it
                            import asyncio

                            if asyncio.iscoroutinefunction(on_chunk):
                                await on_chunk(audio_file, i + 1, len(chunks))
                            else:
                                on_chunk(audio_file, i + 1, len(chunks))
                        except Exception as e:
                            print(f"[TTS] on_chunk callback error: {e}")
                if not result.get("success"):
                    print(f"[TTS] Chunk {i + 1} failed: {result.get('error')}")

            # Return success with all audio files
            successful = [r for r in all_results if r.get("success")]
            if successful:
                result = successful[0]
                result["chunks_total"] = len(chunks)
                result["chunks_succeeded"] = len(successful)
                # Return all audio files for broadcasting
                result["audio_files"] = all_audio_files
            else:
                result = {
                    "success": False,
    ...
    async def _speak_edge_tts(
        ...
        if len(chunks) == 1:
            # Single chunk, no need for special handling
            result = await self.edge_tts_tool.execute(
            ...
            for i, chunk in enumerate(chunks):
                print(
                    f"[Edge TTS] Processing chunk {i + 1}/{len(chunks)}: {len(chunk.split())} words"
                )
                result = await self.edge_tts_tool.execute(
    ...
    def get_available_voices(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_languages": self.XTTS_LANGUAGES
            if self.coqui_tts and "xtts" in self.current_model.lower()
            else ["en"],
        }
    ...
    def get_available_speakers(self) -> list:
                    ...
                    speakers.append(
                        {
                            ...
                            "native_language": speaker_info.get(
        ...
        if self.current_engine == "pyttsx3":
            ...
            if self.pyttsx3_engine:
                try:
                    for idx, voice in enumerate(
                        self.pyttsx3_engine.getProperty("voices")
                    ):
                        speakers.append(
                            {
                                "id": voice.id,
            ...
            if self.current_engine == "coqui":
                if self.coqui_tts and hasattr(self.coqui_tts, "speakers"):
                    if speaker_id in self.coqui_tts.speakers:
                        ...
                        print(
                            f"   ⚠️  Speaker '{speaker_id}' not found in available speakers"
                        )
                        return False
    ...
    def get_current_speaker(self) -> str:
        ...
        if speaker is None:
            ...
            if (
                self.coqui_tts
                and hasattr(self.coqui_tts, "speakers")
                and self.coqui_tts.speakers
            ):
