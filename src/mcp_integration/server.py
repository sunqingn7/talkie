...
class TalkieMCPServer:
    ...
    def __init__(
        ...
        self._external_session_memory = (
            session_memory  # Store external session memory if provided
        )
    ...
    def _register_tools(self):
        ...
        # Warmup TTS during startup
        print("[MCP Server] Warming up TTS...")
        self.tools["speak"].warmup()
        ...
        from tools.voice_daemon_tool import (
            VoiceDaemonStatusTool,
            VoiceDaemonStopTool,
            VoiceDaemonSkipTool,
        )
        ...
        from tools.memory_tools import (
            SearchSessionMemoryTool,
            GetRecentAttachmentsTool,
            GetAttachmentContentTool,
            GetSessionContextTool,
            GetLastUserRequestTool,
        )
        ...
        if self._external_session_memory:
            ...
            print(
                f"[MCP Server] Using external session memory: {self.session_memory.session_id}"
            )
        else:
            self.session_memory = get_session_memory()
            print(
                f"[MCP Server] Created new session memory: {self.session_memory.session_id}"
            )
        ...
        self.tools["read_file_chunk"] = FileReadingTool(
        ...
        self.tools["pause_file_reading"] = PauseFileReadingTool(
            self.tools["read_file_chunk"]
        )
        self.tools["resume_file_reading"] = ResumeFileReadingTool(
            self.tools["read_file_chunk"]
        )
        self.tools["file_reading_status"] = FileReadingStatusTool(
            self.tools["read_file_chunk"]
        )
        self.tools["stop_file_reading"] = StopFileReadingTool(
            self.tools["read_file_chunk"]
        )
