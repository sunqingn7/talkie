"""
Qwen TTS Backend - Local Qwen3-TTS with chunked streaming
"""
...
class LRUCache:
    ...
    def put(self, text: str, speaker: str, language: str, audio: bytes):
        ...
        while (len(self.cache) >= self.max_size or 
               self.total_bytes + len(audio) > self.max_bytes):
            if not self.cache:
                break
...
class QwenTTSBackend(TTSBackend):
    """Qwen3-TTS backend with chunked synthesis for streaming."""
    
    name = "qwen_tts"
    description = "Qwen3-TTS - Local neural TTS with multiple voices (offline)"
    ...
    async def initialize(self) -> bool:
        ...
        import sys
        print(f"[Qwen TTS Backend] Initializing...", flush=True)
        sys.stdout.flush()
    ...
    async def _load_model_background(self):
        ...
        try:
            ...
            if not model_id:
                print(f"[Qwen TTS Backend] Unknown model type: {self.model_type}", flush=True)
                self._load_error = f"Unknown model type: {self.model_type}"
                return
            
            print(f"[Qwen TTS Backend] Loading model in background: {model_id}", flush=True)
            sys.stdout.flush()
            ...
            print(f"[Qwen TTS Backend] Model loaded successfully!", flush=True)
            sys.stdout.flush()
            ...
            print(f"[Qwen TTS Backend] qwen-tts not installed: {e}", flush=True)
    ...
    async def _load_fast_model(self):
        """Load smaller model for hybrid mode."""
        ...
        try:
            ...
            print("[Qwen TTS] Fast model loaded")
    ...
    async def synthesize_stream(
        ...
        **kwargs
        ...
        print(f"[Qwen TTS] Split into {len(chunks)} chunks: {[c[:30]+'...' if len(c)>30 else c for c in chunks]}")
        ...
        for i, chunk in enumerate(chunks):
            ...
            try:
                ...
                use_fast_model = (
                    is_first_chunk and 
                    self._fast_model_for_first and 
                    self._fast_model_loaded and 
                ...
                if pcm_data:
                    ...
                    sub_chunks = self._split_pcm_for_streaming(pcm_data, chunk_size=4096)
                    for sub_chunk in sub_chunks:
                        yield sub_chunk
        ...
        print(f"[Qwen TTS] Cache stats: {stats['hits']} hits, {stats['misses']} misses, {stats['hit_rate']:.1f}% hit rate")
    ...
    async def _synthesize_chunk(
        ...
        """Synthesize a single text chunk to PCM audio."""
        import numpy as np
    ...
    def _split_text_aggressive(
        ...
        for sentence in sentences:
            ...
            for i in range(0, len(clause_parts) - 1, 2):
                clause = clause_parts[i] + (clause_parts[i + 1] if i + 1 < len(clause_parts) else "")
